...
class receive():
    def sniff(self):
        ...
        parse1 = dintext_parse.parse()

        # Here we only test gray failure detection time; DeltaINT-E bandwidth is calculated by software simulation (not here)
        ...
        while True:
            ...
            if rs != None:
                #intpath_prevbw = 0
                #dint_prevbw = 0
                srcip, srcmac, intlist, timedelta_bits, flowkey = rs

                # Convert each element in port_list into str
                for idx in range(len(intlist)):
                    # i=0 -> last hop; i=len(intlist)-1 -> first hop
                    i = len(intlist) - 1 - idx
